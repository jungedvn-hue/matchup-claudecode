-- Notifications v1
-- In-app notification system with realtime support
...
CREATE TABLE IF NOT EXISTS public.notifications (
  ...
  title      TEXT NOT NULL,
  body       TEXT,
  ref_type   TEXT,   -- 'group' | 'event' | 'gift' | 'friend' | 'announcement'
  ref_id     TEXT,
  link       TEXT,   -- in-app deeplink e.g. /group/uuid
  read_at    TIMESTAMPTZ,
...
-- ── RPC: create notification (internal helper, called by other RPCs) ──────────
...
BEGIN
  ...
  VALUES (p_user_id, p_type, p_title, p_body, p_ref_type, p_ref_id, p_link);
END;
$$;

GRANT EXECUTE ON FUNCTION public.fn_notify(UUID,TEXT,TEXT,TEXT,TEXT,TEXT,TEXT) TO authenticated;

-- ── DB Triggers for automatic notifications ───────────────────────────────────

-- 1. group_join_request → notify host when member requests to join
...
BEGIN
  ...
  SELECT g.host_user_id, g.name INTO v_host_id, v_group_name
  FROM public.groups g WHERE g.id = NEW.group_id;

  SELECT COALESCE(display_name, 'Someone') INTO v_requester_name
  FROM public.profiles WHERE user_id = NEW.user_id;

  INSERT INTO public.notifications (user_id, type, title, body, ref_type, ref_id, link)
  VALUES (
    v_host_id, 'group_join_request',
    v_requester_name || ' wants to join',
    v_group_name,
...
-- 2. group_approved → notify member when approved
...
BEGIN
  IF OLD.status = 'pending' AND NEW.status = 'active' THEN
    ...
    INSERT INTO public.notifications (user_id, type, title, body, ref_type, ref_id, link)
    VALUES (
      NEW.user_id, 'group_approved',
      'Request approved 🎉',
      'You are now a member of ' || v_group_name,
      'group', NEW.group_id::TEXT,
      '/group/' || NEW.group_id
    );
  END IF;
...
CREATE TRIGGER trg_notify_approved
  AFTER UPDATE ON public.group_members
  FOR EACH ROW EXECUTE FUNCTION public.trg_notify_approved();

-- 3. announcement posted → notify all active members
...
BEGIN
  ...
  FOR v_member IN
    ...
    WHERE group_id = NEW.group_id AND status = 'active' AND user_id <> NEW.author_id
  LOOP
    INSERT INTO public.notifications (user_id, type, title, body, ref_type, ref_id, link)
    VALUES (
      v_member.user_id, 'group_announcement',
      v_group_name || ': ' || v_title,
      LEFT(NEW.body, 100),
...
-- 4. event created → notify all active members
...
BEGIN
  ...
  FOR v_member IN
    SELECT user_id FROM public.group_members
    WHERE group_id = NEW.group_id AND status = 'active' AND user_id <> NEW.created_by
  LOOP
    INSERT INTO public.notifications (user_id, type, title, body, ref_type, ref_id, link)
    VALUES (
      v_member.user_id, 'event_created',
      'New event in ' || v_group_name,
      NEW.title,
      'event', NEW.id::TEXT,
      '/group/' || NEW.group_id
    );
...
BEGIN
  ...
  INSERT INTO public.notifications (user_id, type, title, body, ref_type, ref_id, link)
  VALUES (
    NEW.to_user_id, 'gift_received',
    v_sender_name || ' gifted you a drink! ' || NEW.item_emoji,
    NEW.item_name || ' · ' || NEW.coins_item || ' coins',
    'gift', NEW.id::TEXT,
    '/wallet'
  );
  RETURN NEW;
...
CREATE TRIGGER trg_notify_drink_gift
  AFTER INSERT ON public.drink_gifts
  FOR EACH ROW EXECUTE FUNCTION public.trg_notify_drink_gift();

-- 6. coin gift received (fn_send_gift inserts into coin_transactions — trigger on that)
...
BEGIN
  ...
  -- Find the matching sent transaction to get sender
  SELECT user_id INTO v_sender_id FROM public.coin_transactions
  WHERE ref_id = NEW.ref_id AND type = 'gift_sent' LIMIT 1;
  ...
  INSERT INTO public.notifications (user_id, type, title, body, ref_type, ref_id, link)
  VALUES (
    NEW.user_id, 'gift_received',
    v_sender_name || ' sent you ' || NEW.amount || ' coins 🎁',
    NULL, 'wallet', NEW.ref_id, '/wallet'
  );
  RETURN NEW;
END;
$$;
...
CREATE TRIGGER trg_notify_coin_gift
  AFTER INSERT ON public.coin_transactions
  FOR EACH ROW EXECUTE FUNCTION public.trg_notify_coin_gift();

-- 7. friend_request + friend_accepted (on friendships table)
