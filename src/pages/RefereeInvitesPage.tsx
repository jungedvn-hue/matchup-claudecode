...
import { ArrowLeft, Gavel, Loader2, Check, X, Clock, CalendarClock } from "lucide-react";
...
const STATUS_META: Record<string, { color: string; label: string }> = {
  pending:  { color: "bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/30", label: "Pending" },
  accepted: { color: "bg-primary/10 text-primary border-primary/30", label: "Accepted" },
  declined: { color: "bg-muted text-muted-foreground border-border", label: "Declined" },
  expired:  { color: "bg-muted text-muted-foreground border-border", label: "Expired" },
};

const RefereeInvitesPage = () => {
    ...
    <div className="min-h-screen pb-20">
      <div className="sticky top-0 z-40 bg-background/90 backdrop-blur-lg border-b border-border px-4 py-3">
        <div className="flex items-center gap-3 max-w-2xl mx-auto">
          ...
          <h1 className="text-lg font-display font-bold flex items-center gap-2">
            <Gavel className="h-5 w-5 text-primary" /> {t("refInvites.title")}
          </h1>
        </div>
      </div>
      ...
      <div className="px-4 pt-4 max-w-2xl mx-auto space-y-4">
        {loading ? (
          ...
          <Card className="p-8 text-center text-sm text-muted-foreground">{t("refInvites.empty")}</Card>
        ) : (
          <>
            {pending.length > 0 && (
              <div>
...
export default RefereeInvitesPage;
